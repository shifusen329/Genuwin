...
public class TouchManager {
    ...
    public boolean isTap() {
        return calculateGetFlickDistance() < 0.1f;
    }

    public boolean isDoubleTap() {
        long currentTime = System.currentTimeMillis();
        if ((currentTime - _lastTapTime) < 500) {
            // A tap has occurred within 500ms of the last tap.
            return true;
        }
        return false;
    }

    public void updateTapTime() {
        _lastTapTime = System.currentTimeMillis();
    }
    
    /**
     * Check if this is a single tap (not part of a double tap sequence)
     * This should be called after a delay to ensure it's not part of a double tap
     */
