...
public class GoogleDriveTool implements ToolExecutor {
    private static final String TAG = "GoogleDriveTool";
    ...
    public GoogleDriveTool(Context context) {
    ...
    private void loadConfig() {
        ...
        try {
            ...
            this.baseUrl = properties.getProperty("GOOGLE_DRIVE_BASE_URL");
            if (this.baseUrl == null || this.baseUrl.trim().isEmpty()) {
                Log.e(TAG, "GOOGLE_DRIVE_BASE_URL not configured in config.properties");
            }
        } catch (Exception e) {
    ...
    public ToolDefinition getToolDefinition() {
        ...
        params.put("query", new ToolDefinition.ParameterDefinition("string", "Search query", true));
        return new ToolDefinition("search_gdrive", "Search for files in Google Drive", params);
    }
            ...
            try {
                ...
                try (Response response = client.newCall(request).execute()) {
                    if (response.isSuccessful()) {
                        ...
                        callback.onError("Failed to search Google Drive: " + response.message());
                    }
                }
            } catch (Exception e) {
                Log.e(TAG, "Error executing Google Drive search", e);
                callback.onError("Error executing Google Drive search: " + e.getMessage());
            }
        }).start();
    }
}
